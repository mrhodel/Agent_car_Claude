...
Adds gimbal + (optionally) chassis motors while keeping the heavy stack off.
Motor movement is gated behind --enable-motors for safety.

Two key gimbal experiments are supported here too:
  - Small-range sweep:   --pan-min -20 --pan-max 20
  - End-stops only:      --endstop-only

Usage:
  python bench_camera_motion.py --duration 60            # camera + gimbal only
  python bench_camera_motion.py --duration 60 --enable-motors
...
def main() -> int:
    ...
    ap.add_argument("--motor-speed", type=int, default=20, help="Percent (0-100), small by default")
    ap.add_argument("--segment", type=float, default=0.6, help="Seconds per motion segment")

    ap.add_argument("--pan-step", type=float, default=45.0)
    ap.add_argument("--dwell", type=float, default=0.20)
    ap.add_argument("--endstop-only", action="store_true")
    ap.add_argument("--pan-min", type=float, default=None)
    ap.add_argument("--pan-max", type=float, default=None)
    ...
    if args.enable_motors:
        ...
        motors.stop()
        print("[Motion] Motors ENABLED", flush=True)
    else:
        print("[Motion] Motors disabled (use --enable-motors)", flush=True)
    ...
    print(
        f"[MotionBench] pan_min={pan_min:.1f} pan_max={pan_max:.1f} positions={len(positions)} endstop_only={bool(args.endstop_only)}",
        flush=True,
    )
        ...
        while True:
            ...
            if now >= next_print:
                ...
                print(
                    ...
                    f"pan={gimbal.current_pan:6.1f}  motion={motion_name:5s}  unchanged_for={frozen_for:4.1f}s{freeze_flag}",
                    flush=True,
                )
